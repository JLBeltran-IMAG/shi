...
def execute_SHI(path_to_images, path_to_result, mask_period, flat):
    ...
    for path in path_to_images:
        ...
        wavevector_kx, wavevector_ky, fft_img = spatial_harmonics.squared_fast_fourier_transform_linear_and_logarithmic(img, mask_period)
        harmonics, labels = spatial_harmonics.spatial_harmonics_of_fourier_spectrum(fft_img, wavevector_ky, wavevector_kx, flat)
        ...
        for idx in range(1, len(labels)):
            scattering = spatial_harmonics.contrast_retrieval_individual_members(harmonics[idx], type_of_contrast="scattering")
            directories.export_result_to(scattering, path.stem + "_" + labels[idx], path_to_result, "scattering")

            phasemap = spatial_harmonics.contrast_retrieval_individual_members(harmonics[idx], type_of_contrast = "phasemap", label = labels[idx])
...
def my_parser():
    ...
    parser_clean = subparsers.add_parser("clean", help="This subcommand deletes temporary files generated by the SHI method to avoid memory shortages.")
    ...
    parser_shi.add_argument("--all-2d", action="store_true", help="Executes the SHI-2D method in the current folder without specifying inputs")
    parser_shi.add_argument("--all-3d", action="store_true", help="Executes the SHI-CT method in the current folder without specifying inputs")
    ...
    if args.comando == "calculate":
        ...
        for image_path in images_path:
            ...
            if dark_path is not None:
                corrections.correct_darkfield(path_to_dark=dark_path, path_to_images=image_path, crop=crop_from_tmptxt, allow_crop=True, angle=deg)
                if flat_path is not None:
                    corrections.correct_darkfield(path_to_dark=dark_path, path_to_images=flat_path, crop=crop_from_tmptxt, allow_crop=True, angle=deg)
                ...
                corrections.crop_without_corrections(path_to_images=image_path, crop=crop_from_tmptxt, allow_crop=True, angle=deg)
            ...
            if flat_path is None:
                ...
                corrections.crop_without_corrections(path_to_images=flat_path, crop=crop_from_tmptxt, allow_crop=True, angle=deg)
                ...
                path_to_flat, path_to_flat_result = directories.create_result_subfolders(
                    directory_for_files=path_to_corrected_flat, folders_for_result=path_to_result.name, sample_folder_name="flat"
                )
                ...
                path_to_average_flat_absorption = math_utils.average_flat_harmonics(path_to_flat_result, type_of_contrast="absorption")
                path_to_average_flat_scattering = math_utils.average_flat_harmonics(path_to_flat_result, type_of_contrast="scattering")
                path_to_average_flat_phase = math_utils.average_flat_harmonics(path_to_flat_result, type_of_contrast="phase")
                path_to_average_flat_phasemap = math_utils.average_flat_harmonics(path_to_flat_result, type_of_contrast="phasemap")

                if args.all_2d:
                    corrections.correct_flatmask(path_to_result, path_to_average_flat_absorption, type_of_contrast="absorption")
                    corrections.correct_flatmask(path_to_result, path_to_average_flat_scattering, type_of_contrast="scattering")
                    ...
                    corrections_ct.correct_flatmask(path_to_result, path_to_average_flat_absorption, type_of_contrast="absorption")
                    corrections_ct.correct_flatmask(path_to_result, path_to_average_flat_scattering, type_of_contrast="scattering")
                    corrections_ct.correct_flatmask(path_to_result, path_to_average_flat_phase, type_of_contrast="phase")
                    corrections_ct.correct_flatmask(path_to_result, path_to_average_flat_phasemap, type_of_contrast="phasemap")
            ...
            if args.average:
                commands = [
                    ["python", "avg.py", "--path", path_to_result.joinpath("absorption", "flat_corrections").as_posix(), "-t", "absorption"],
                    ["python", "avg.py", "--path", path_to_result.joinpath("scattering", "flat_corrections").as_posix(), "-t", "scattering"],
                    ["python", "avg.py", "--path", path_to_result.joinpath("phase", "flat_corrections").as_posix(), "-t", "phase"],
                    ["python", "avg.py", "--path", path_to_result.joinpath("phasemap", "flat_corrections").as_posix(), "-t", "phasemap"],
                ]

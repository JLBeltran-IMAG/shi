...
def load_tiff_image(file_path: str) -> Image.Image:
    """
    Carga una imagen TIFF de 16 bits.
    ...
    -------
    np.ndarray
        Imagen cargada como un array de NumPy.
    """
    # In the future, I will use metadata for different purposes
    # For now, just load the image
    with ti.TiffFile(file_path) as tif:
        image = tif.asarray()
    return normalize_image_for_display(image)
...
def cropImage(path_imagen: Path):
    app = ROISelector(path_imagen)
    app.mainloop()
    return app.roi_coords


class ROISelector(tk.Tk):
    def __init__(self, image_path, display_max_size=800):
        super().__init__()
        # GUI Title
        self.title("Region of Interest")

        # Open image
        self.image = load_tiff_image(image_path)

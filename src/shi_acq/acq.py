...
class AcqViewerApp:
    ...
    def _start_ct_acquisition(self):
        ...
        interval = self.text_box_angle_min.get("1.0", "end-1c") + "-" + self.text_box_angle_max.get("1.0", "end-1c")
        proj2d_no = int(self.spinbox_ct.get())

        nanodeg = 1_000_000_000
        angle_step = int((max_angle - min_angle) / proj2d_no * nanodeg)

        # Path for saving CT scan
        ctpath = self._sample_dir.joinpath(f"ct_{self.text_box_ct.get("1.0", "end-1c")}")
        ...
        if not self._update_active:
            ...
            log.info(f"COMPUTED TOMOGRAPHY STARTED")
            for imgIndex in range(numImages):
                # ----------------------------------------------------------------------------------
                if imgIndex == 0:
                    if min_angle == 0:
                    ...
                    motor_control.moving_motor(d_handle, channel, move_mode, angle_step)
                    angle_save += int(ctl.GetProperty_i64(d_handle, channel, ctl.PropertyKey.POSITION))
                # ----------------------------------------------------------------------------------
                log.info(f"Img #{imgIndex} x {angle_step} == {angle_save}")

                status = pygigev.GevWaitForNextFrame(self.handle, ctypes.byref(gevbufPtr), tmout.value)
                log.info(f"Frame status -> {status}")
                ...
                if gevbuf.status != 0:
                    log.info(f"Img #{index} status = {gevbuf.status} GEV_FRAME_STATUS_ERROR")
                    continue
                else:
                    log.info(f"Img # {index} has status = {gevbuf.status} GEV_FRAME_STATUS_RECVD")
                ...
                filepath = ctpath.joinpath(filename)
                io.imsave(filepath, image)
                index += 1
                log.info(f"File {self.text_box_ct.get("1.0", "end-1c")}{index:03d} already saved")
            ...
            status = pygigev.GevReleaseFrame(self.handle, gevbufPtr)
            log.info(f"Free Frame status = {status} error")

            status = pygigev.GevFreeTransfer(self.handle)
            if status == 0:
                messagebox.showinfo("Success", "CT-projections were successfully saved")
                log.info(msgs_.free_transfer)
            else:
                messagebox.showwarning(
                    "Warning",
            ...
            log.info(f"COMPUTED TOMOGRAPHY STOPPED")

...
from .exceptions import ImageNotFoundError, ProcessingError
from .logging import logger

from .. import spatial_harmonics
...
class SHIProcessor:
    ...
    def process_directory(
        ...
        Args:
            images_path: Path to sample images
    ...
    def _process_single_image(
        ...
        deg = self._get_angle_correction(image_path, flat_path) if angle_after else np.float32(0)
        ...
        if dark_path:
            ...
            self._apply_dark_corrections(dark_path, flat_path, image_path, bright_path, crop_from_tmptxt, deg)
            ...
            self._apply_crop_only(image_path, flat_path, crop_from_tmptxt, deg)
            foldername_to = "crop_without_correction"
            
        # Create result directories and process
        path_to_corrected = image_path.parent / foldername_to
        path_to_images, path_to_result = directories.create_result_subfolders(
            file_dir=str(path_to_corrected),
        ...
        if not flat_path:
            spatial_harmonics.execute_SHI(path_to_images, path_to_result, self.mask_period, self.unwrap_method, False)
        else:
            self._process_with_flat(path_to_corrected, flat_path, path_to_result, mode, average)
    ...
    def _get_angle_correction(self, image_path: Path, flat_path: Optional[Path]) -> np.float32:
    ...
    def _process_with_flat(
        ...
        spatial_harmonics.execute_SHI(path_to_flat, path_to_flat_result, self.mask_period, self.unwrap_method, True)
        spatial_harmonics.execute_SHI(str(corrected_path), result_path, self.mask_period, self.unwrap_method, False)
        ...
        for contrast in config.CONTRAST_TYPES:
            path_to_average_flat = directories.average_flat_harmonics(path_to_flat_result, contrast)
            corrections.correct_flatmask(result_path, path_to_average_flat, contrast)
